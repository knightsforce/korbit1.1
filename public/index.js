...
var templates = {//Быстрее создать один раз, а потом клонировать, для инкапсуляции не прописал их в html
	...
	"span": $("<span>"),
	"a": $("<a>"),
	"action_template": $(".actions_product.hide"),
	"h6": $("<h6>"),
	"form": $("<form>"),
	"ul": $("<ul>"),
	"li": $("<li>"),
	"checkbox": $("<div class='checkbox'><span></span></div>"),
	"label": $("<label>"),
...
var slider_price_track = sort_price.find(".track");
...
var collection_checkbox = sort_options.find(".checkbox");
...
body.on("click", function(e) {
	...
	*/
	...
	if(sort_header.length) {
		...
		checbox.toggleClass("active");
		filteringProducts("women");

		var count_active = checbox.closest(".sort_list").find(".checkbox.active").length;
		...
		var all_links = navigation.find(".link");
...
.on("mousedown.range", ".slider", function(e) {
	/*
...
//Обращение к переменной быстрее, чем к цепочке объектов
	...
	body.on("mousemove.range", function(e) {//движение на body
		...
		$(this).on("mouseup.range", function(e) {//при отпускании клавиши, ненужные обработчки движения и отпускания снимаются
			...
			var clear = slider.closest(".sort_list").find(".clear");
...
function filteringProducts(gender_key) {
	...
	var active_checkbox = collection_checkbox.filter(".active");
...
function showResultSearch(opt) {
	search_result.text("Всего найдено "+opt["count"]);
}
...
function createProductsDOM(arr, rootEl) {//По хорошему все рендерится на сервере и при надобности подгружается через ajax или websocket
	...
	*/
	...
	var products_created = arr.map((item)=>{
		...
		img_wrap.append(action_template.clone(true).removeClass("hide"))
		.append(img);
...
function createSortList(values, rootElem) {
	...
	*/
	...
	var clear = span.clone().addClass("clear").text(values["clear_text"]);
	...
	*/
		...
		var slide_elems=[div.clone().addClass("track")];
		...
		switch(type) {
			case "two":
				...
				slide_elems.push(div.clone().addClass("slider range_max"));
			...
			case "one":
				...
				slide_elems.push(range_min = div.clone().addClass("slider range_min"));
...
function createPageNavigation(products){
	navigation_wrap.find(":not(.link_left,.link_right,.spread_left,.spread_right)").remove();//При фильтрации нужно перерисовывать
	//Просто отрисовка
	var products = (products) ? $(products) : $(".product:visible");
	var count = products.length;
	//var v_link = 5; //Видимые ссылки
	...
	*/
	...
	if(min_count>=(count>>1)) {//минимальное количество отображаемых элементов >= половине всех элементов
		...
		*/
		link_right.prop("num", 3).text(3);
		templates["div"].clone().addClass("link").prop("num", 2).text(2).insertAfter(spread_left);
		pages_count = 3;
		return;
	} else {
		...
		//Если <4 при элементах больших 4 страниц, то получается бред
		...
		for(let i = 0; i<count; i++) {
			...
			(i!=0) && arr.push(div.clone().addClass("link").prop("num", num).text(num));
		}
		arr.pop();
		spread_left.after(arr);
		link_right.prop("num", num).text(num);
		pages_count = num;
	}
}
...
function clickPageNavigation(num, links) {
	...
	*/
	...
	var old_num = num;
	var all_links = navigation_wrap.find(".link");
	var links = all_links.not(".link_left,.link_right,.spread_left,.spread_right");
		...
		switch(num) {
			case first_num:
				for (let i=count-1; i>=0; i--) {
					--last_num;
					item = links.eq(i);
					item.prop("num", last_num).text(last_num);
				}
			...
			case last_num:
				for (let i=0; i<count; i++) {
					++first_num;
					item = links.eq(i);
					item.prop("num", first_num).text(first_num);
				}
...
/*
Старая уже не рабочая версия

function clickPageNavigation(num, links) {
	//Переделка при клике

	//	Проблемы когда v-l\link = 2, т.к. код приспособлен для любых величин, кроме этой,
	//	обработаю вручную
	//	Много времени потратил на отображение любого количества, для фиксированного было бы проще
/
	...
	var count = links.length;
	all_links.filter(".active").removeClass("active");
	//var _links = links.filter(".left_link, .spread_left, .spread_right");
	
	//	Вместо того, чтобы проходить 3 раза всю коллекцию,
	//	1 раз прохожу всю и ищу нужные единицы, которых мало
	//	и из них выбираю
	var left_num = link_left.prop("num");
	var right_num = link_right.prop("num");
	...
	switch(num) {
		case first_num:
			...
			if(first_num-n==left_num) {
				(!p) && link_left.addClass("hide");
				spread_left.addClass("hide");

			} else if(first_num == left_num) {
			...
			link_right.removeClass("hide");
			spread_right.removeClass("hide");
			//log("fn", num, first_num);
			break;

		case last_num:
			
			if(last_num+n==right_num) {

				(!p) && link_right.addClass("hide");
				spread_right.addClass("hide");
			
			} else if(last_num == right_num) {

			}

			help_bottom(count);
			
			link_left.removeClass("hide");
			spread_left.removeClass("hide");
			//log("ln", num, first_num);
			break;

		default:
			//log(num)
			//links.eq().addClass("active");
			break;
	}
	//Решение так себе, но работает быстро из-за априори малого количества элементов
	all_links.each((i,item)=>{
		item = $(item);
		if(item.prop("num")==num) item.addClass("active");
	});
	
	//log(num, left_num+p, right_num-p);
	function help_top(count) {
		for (let i=count-1; i>=0; i--) {
			if(num == left_num+p) break;
			//if (v_link<=2 && last_num==Y)
			--last_num;
			item = links.eq(i);
			item.prop("num", last_num).text(last_num);
		}
	...
	function help_bottom(count) {
		for (let i=0; i<count; i++) {
			if(num == right_num-p) break;
			++first_num;
			item = links.eq(i);
			item.prop("num", first_num).text(first_num);
		}
...
function resetOptions(parent) {
	parent.each(function(i, item) {
		var checkbox = parent.find(".checkbox").removeClass("active");
		item = $(item);
		item.find(".clear").css("display", "none");
		/*
